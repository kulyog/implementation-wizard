...
import { buildAllPersonasText } from '../../data/supportPersonas'
...
Please review the incomplete steps above and help me continue from where I left off. Start by examining any step currently In Progress, or identify the next step that should be worked on based on sequential order (Steps 5 and 6 may run in parallel once Step 4 is complete).`
  ...
  return (
    <div className="h-full overflow-y-auto px-6 py-5 space-y-6">
      ...
      {/* Step 0 — Copy All Definitions */}
      {staticStep.step_number === 0 && (
        <div>
          <button
            onClick={async () => {
              const ok = await copyAllDefs(buildAllPersonasText())
              if (ok) onCopied('All 8 persona definitions copied. Paste into Claude Web → Project Settings → Project Instructions.')
            }}
            className="w-full bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium px-4 py-2 rounded-md transition-colors"
          >
            {copiedAllDefs ? 'Copied! ✓' : 'Copy All Definitions'}
          </button>
          <p className="mt-2 text-xs text-gray-500">
            Copy all persona definitions and paste into Claude Web Project Settings → Project Instructions before marking this step complete.
          </p>
        </div>
      )}
